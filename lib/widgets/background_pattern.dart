import 'dart:math' as math;
import 'package:flutter/material.dart';

class BackgroundPattern extends StatefulWidget {
  static const Color _patternPink = Color(0xFFF4C8D3);
  static const Color _patternYellow = Color(0xFFF6E6A8);
  static const Color _patternPeach = Color(0xFFF6CDBB);
  static const Color _patternPrimary = Color(0xFFA8CFA3);
  static const Color _patternPrimaryDark = Color(0xFF6F9E6E);

  const BackgroundPattern({super.key});

  @override
  State<BackgroundPattern> createState() => _BackgroundPatternState();
}

class _BackgroundPatternState extends State<BackgroundPattern> {
  Size? _cachedSize;
  Widget? _cachedPattern;

  @override
  Widget build(BuildContext context) {
    final size = MediaQuery.sizeOf(context);
    if (_cachedPattern == null || _cachedSize != size) {
      _cachedSize = size;
      _cachedPattern = _buildPattern(size);
    }

    return IgnorePointer(
      child: RepaintBoundary(
        child: OverflowBox(
          maxWidth: size.width,
          maxHeight: size.height,
          alignment: Alignment.topCenter,
          child: _cachedPattern,
        ),
      ),
    );
  }

  Widget _buildPattern(Size size) {
    final width = size.width;
    final height = size.height;

    return SizedBox(
      width: width,
      height: height,
      child: Stack(
        clipBehavior: Clip.none,
        children: [
          _buildPatternIcon(
            width,
            height,
            Icons.favorite,
            BackgroundPattern._patternPink,
            0.7,
            48,
            0.05,
            0.12,
            -15,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.menu_book,
            BackgroundPattern._patternPrimaryDark,
            0.6,
            36,
            0.18,
            0.06,
            35,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.star_rounded,
            BackgroundPattern._patternYellow,
            0.8,
            30,
            0.08,
            0.28,
            20,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.schedule,
            BackgroundPattern._patternPrimary,
            0.6,
            44,
            0.22,
            0.22,
            -25,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.lightbulb,
            BackgroundPattern._patternPeach,
            0.7,
            28,
            0.32,
            0.08,
            15,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.local_florist,
            BackgroundPattern._patternPeach,
            0.7,
            48,
            0.08,
            null,
            15,
            right: 0.15,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.cloud,
            BackgroundPattern._patternPrimaryDark,
            0.5,
            40,
            0.22,
            null,
            -10,
            right: 0.25,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.auto_awesome,
            BackgroundPattern._patternYellow,
            0.8,
            32,
            0.06,
            null,
            45,
            right: 0.35,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.filter_vintage,
            BackgroundPattern._patternPink,
            0.6,
            36,
            0.30,
            null,
            -20,
            right: 0.08,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.wb_sunny,
            BackgroundPattern._patternYellow,
            0.6,
            26,
            0.15,
            null,
            30,
            right: 0.05,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.eco,
            BackgroundPattern._patternPrimaryDark,
            0.6,
            48,
            null,
            0.08,
            25,
            bottom: 0.12,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.edit,
            BackgroundPattern._patternPink,
            0.6,
            38,
            null,
            0.25,
            -15,
            bottom: 0.22,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.note,
            BackgroundPattern._patternPeach,
            0.7,
            44,
            null,
            0.30,
            15,
            bottom: 0.05,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.emoji_objects,
            BackgroundPattern._patternYellow,
            0.7,
            30,
            null,
            0.12,
            -10,
            bottom: 0.30,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.flag,
            BackgroundPattern._patternPrimary,
            0.5,
            34,
            null,
            0.05,
            5,
            bottom: 0.40,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.local_cafe,
            BackgroundPattern._patternPrimary,
            0.7,
            48,
            null,
            null,
            -20,
            bottom: 0.18,
            right: 0.18,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.sentiment_satisfied_alt,
            BackgroundPattern._patternYellow,
            0.7,
            38,
            null,
            null,
            30,
            bottom: 0.08,
            right: 0.35,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.check_circle,
            BackgroundPattern._patternPrimaryDark,
            0.6,
            44,
            null,
            null,
            10,
            bottom: 0.28,
            right: 0.10,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.headset,
            BackgroundPattern._patternPink,
            0.6,
            32,
            null,
            null,
            -15,
            bottom: 0.05,
            right: 0.10,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.directions_run,
            BackgroundPattern._patternPeach,
            0.6,
            40,
            null,
            null,
            20,
            bottom: 0.40,
            right: 0.05,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.calendar_month,
            BackgroundPattern._patternPeach,
            0.6,
            32,
            0.38,
            0.10,
            45,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.favorite,
            BackgroundPattern._patternPink,
            0.6,
            40,
            null,
            null,
            -30,
            bottom: 0.45,
            right: 0.15,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.bolt,
            BackgroundPattern._patternYellow,
            0.7,
            36,
            0.45,
            null,
            15,
            right: 0.25,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.push_pin,
            BackgroundPattern._patternPrimaryDark,
            0.5,
            30,
            null,
            0.35,
            -20,
            bottom: 0.35,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.work_outline,
            BackgroundPattern._patternPrimary,
            0.6,
            34,
            0.15,
            0.40,
            -10,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.code,
            BackgroundPattern._patternYellow,
            0.5,
            42,
            0.25,
            0.60,
            20,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.brush,
            BackgroundPattern._patternPink,
            0.7,
            30,
            0.35,
            0.45,
            -25,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.rocket_launch,
            BackgroundPattern._patternPeach,
            0.6,
            38,
            0.55,
            0.25,
            15,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.extension,
            BackgroundPattern._patternPrimaryDark,
            0.5,
            36,
            0.65,
            0.75,
            -30,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.mic,
            BackgroundPattern._patternPrimary,
            0.6,
            28,
            0.80,
            0.50,
            10,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.music_note,
            BackgroundPattern._patternYellow,
            0.7,
            34,
            0.75,
            0.30,
            -15,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.coffee,
            BackgroundPattern._patternPeach,
            0.6,
            40,
            0.10,
            0.70,
            25,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.spa,
            BackgroundPattern._patternPink,
            0.7,
            32,
            0.45,
            0.80,
            -10,
          ),
          _buildPatternIcon(
            width,
            height,
            Icons.sports_esports,
            BackgroundPattern._patternPrimaryDark,
            0.6,
            38,
            0.85,
            0.80,
            35,
          ),
        ],
      ),
    );
  }

  Widget _buildPatternIcon(
    double maxWidth,
    double maxHeight,
    IconData icon,
    Color color,
    double opacity,
    double size,
    double? topPos,
    double? leftPos,
    double rotationDegrees, {
    double? right,
    double? bottom,
  }) {
    return Positioned(
      top: topPos != null ? maxHeight * topPos : null,
      left: leftPos != null ? maxWidth * leftPos : null,
      right: right != null ? maxWidth * right : null,
      bottom: bottom != null ? maxHeight * bottom : null,
      child: Transform.rotate(
        angle: rotationDegrees * (math.pi / 180),
        child: Icon(
          icon,
          color: color.withAlpha((255 * opacity).round()),
          size: size,
        ),
      ),
    );
  }
}
